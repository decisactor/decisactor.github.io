...
function Import-Docx () 
{
    ...
    $filePath = "$HOME\Downloads\TOEFL\TPO\TPO$tpoNumber\TPO$tpoNumber "
...
$global:projectPath = "$HOME\Downloads\ETS\TOEFL\TOEFLSampler\forml1"
$global:default = [Type]::Missing
$global:wavPath = "$HOME\Music\*"
$global:jpgPath = "$HOME\Downloads\ETS\TOEFL\TPO\resources\cache\*"
...
for ($i = 50; $i -lt 52; $i++) 
{
    #$file = $word.Documents.Open("$HOME\Documents\TOEFL\TPO\TPO$i\TPO$i Speaking.docx")
    $range = $file.Content
    for ($j = 3; $j -lt 7; $j++)
    {
